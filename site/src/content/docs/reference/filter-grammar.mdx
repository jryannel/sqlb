...
| `expand` | Relations to resolve inline |
...
|---|---|---|
| `DefaultPageSize` | 25 | `schema.REST{DefaultPageSize}` |
| `MaxPageSize` | 200 | `schema.REST{MaxPageSize}` |
| `MaxFilters` | 24 | `schema.REST{MaxFilters}` |
| `MaxGroupDepth` | 3 | package constant |

A page size above the maximum is **capped rather than rejected** — a client
asking for too much gets the maximum. Exceeding `MaxFilters` or the sort-term
limit *is* a rejection, naming the count and the limit.

`page` below 1 and a negative `offset` are both rejected.
...
[ADR-0027](/sqlb/adr/0027-keyset-pagination/).
