---
title: Glossary
description: The words sqlb uses for its own machinery, each with the page that explains it properly.
sidebar:
  label: Glossary
  order: 7
---
...
schema language to keep in sync. See [Declaring tables](/sqlb/schema/).
...
reference](/sqlb/reference/column-types/).
...
[Capabilities](/sqlb/concepts/capabilities/) and the [capability
reference](/sqlb/reference/capabilities/).
...
name](/sqlb/project/architecture/#the-wire-is-the-column-name).
...
required](/sqlb/project/architecture/#declared-scope-is-required).
...
producers](/sqlb/concepts/one-grammar/).
...
value](/sqlb/concepts/queries-are-values/).
...
state. See [Where domain logic goes](/sqlb/concepts/domain-logic/).
...
columns](/sqlb/queries/typed-columns/).
...
[Expanding relations](/sqlb/rest/expand/).
...
reference](/sqlb/reference/rejections/).
...
array column cannot be `Sortable()`. See [Pagination](/sqlb/rest/pagination/).
...
See [Adopting a database](/sqlb/migrations/adopting/).
...
compatibility](/sqlb/rest/compatibility/).
...
needed. See [Ejecting](/sqlb/project/eject/).
...
[architecture.md](/sqlb/project/architecture/#decisions) with the alternative it
