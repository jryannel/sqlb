...
| `N filters requested, the limit is M` | `MaxFilters` |
| `N sort terms requested, the limit is M` | The sort-term limit |
...
| 409 | A constraint the application chose to state — a unique index, or a domain rule a hook turned into a refusal |
...
See [ADR-0011](/sqlb/adr/0011-actionable-errors/) for why rejections carry data
rather than prose.
