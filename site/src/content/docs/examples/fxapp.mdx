...
([ADR-0030](/sqlb/adr/0030-declared-scope-is-required/)), the operation set
...
([ADR-0016](/sqlb/adr/0016-guards-proven-both-ways/)).

**That ordering is better expressed as a type than as a list.** `dbbase.Migrated`
...
filter on. Both ask for `name:"unscoped"`, so `grep` lists every consumer — a
"skip the hooks" boolean would instead be something any caller could pass, and
which callers may pass it is the whole question.
...
Four platform modules that would be the same in any application — `logs`,
`dbbase`, `sqlbkit`, `httpkit` — and four that are this one: `store`, `access`,
