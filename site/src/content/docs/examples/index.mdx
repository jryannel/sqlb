---
title: Examples
description: Seven worked applications built on sqlb, and two more built the other way round for comparison — what each one proves, and which to read first.
sidebar:
  label: All examples
  order: 0
---
...
<CardGrid>
  <Card title="fxapp — the wiring" icon="puzzle">
...
| [fxapp](/sqlb/examples/fxapp/) | a key per tenant | Registering the hooks before anything is mounted | A value group, so the handle cannot exist until every module has contributed |
