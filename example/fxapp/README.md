...
The answer, since [ADR-0044](../../docs/adr/0044-the-container-is-an-adapter.md),
is [`sqlbfx`](../../sqlbfx/): the pool, the migration runner, the hook
registry, the two handles and the HTTP surface are the kit's, and what is left
here is what is genuinely this application's — a schema, its hooks, an auth
module, and the configuration boundary in [`platform.go`](platform.go). This
example used to hand-write that glue in three packages (`dbbase`, `sqlbkit`,
`httpkit`); they were promoted into the kit, and the example is now what a
consumer of the kit looks like.
...
| [`access/middleware.go`](access/middleware.go) | The auth module: verify a key, store the principal through the sqlbfx seam. |
| [`app_test.go`](app_test.go) | Every claim here, asserted against a real Postgres — including the composition that must fail. |

## The shape of it

```
logs ──── sqlbfx kit ── Migrated ─┬─ Unscoped handle ── spaces.Directory ─┐
platform.go (configs) ─┘          │                                       │
                                  └───────────── scoped handle ◀── hooks ─┘
                                                       │
...
`sqlbfx.ProvideHooks`, and the handle everything queries through cannot be
...
raises when a resource declares a scope no hook backs, `sqlbfx.OperationSet`
...
**Ordering is a type, not a position.** `sqlbfx.Migrated` is an empty struct
...
uses and `sqlbfx.Unscoped` for the two jobs that cannot be scoped because
...
whole point. `grep -r 'sqlbfx.Unscoped'` lists every consumer.
...
| | |
|---|---|
| `Platform()` ([`platform.go`](platform.go)) | The process logger, and the sqlbfx kit fed by this app's env: pool, migrations, handles, chi + Huma, server lifetime. |
...
verify, then `sqlbfx.WithPrincipal` — the hooks read the principal back
...
an argument. `sqlbfx` is an adapter in a module of its own; what the container
buys is that the ordering constraints become edges the compiler and the boot
enforce, and the cost is a layer of indirection between a constructor and its
caller. Both are real; which one is worth it depends on how many modules the
application has.
...
where that changes: take `sqlbfx.Pool()` and `sqlbfx.Handles()` without
`sqlbfx.Migrations()`, and supply the `Migrated` fact from whatever your
