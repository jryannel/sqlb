...
//
// It provides *pgxpool.Pool and nothing sqlb-specific: sqlb takes an Executor
// and never opens a connection of its own, so how the pool is sized and where
// its DSN comes from is this application's business and no part of the
// library's (ADR-0040). The sqlb handle is built one layer up, in sqlbkit.
//
