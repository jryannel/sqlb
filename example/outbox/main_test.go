...
// The bootstrap is copied from example/fxapp/main_test.go rather than shared:
// every one of these lean examples is a standalone module by design (its own
// go.mod, its own gate), so the alternative is a module whose only purpose is
// a helper the others would import — more machinery than it would save.
...
// freshDatabase returns the DSN of an empty database. Empty, not migrated:
// applying outboxschema's tables is the caller's job (outboxDB, in
// outbox_test.go), the same split example/fxapp/main_test.go uses.
func freshDatabase(t *testing.T) string {
	t.Helper()
	once.Do(startPostgres)
	if startErr != nil {
		t.Fatalf("outbox: %v", startErr)
	}
