...
//
// The filter package's own tests compile `?metadata=contains.{"lang":"de"}` to
...
// Doc is the model for a table with a document column.
type Doc struct {
...
// docsRegistry declares the table Doc maps to, with the GIN index that makes
// containment an index scan rather than a table scan.
func docsRegistry() *schema.Registry {
...
// seedDocs inserts three rows whose metadata overlaps deliberately: one exact
...
func seedDocs(t *testing.T, db *sql.DB) {
	...
	for _, row := range []struct{ title, metadata string }{
		...
		if _, err := db.Exec(
			`INSERT INTO docs (title, metadata) VALUES ($1, $2::jsonb)`, row.title, row.metadata,
		); err != nil {
...
// docsQuery parses a query string against the Doc model and applies it.
func docsQuery(t *testing.T, query string) *sqlb.Builder[Doc] {
	t.Helper()
	values, err := url.ParseQuery(query)
	if err != nil {
		t.Fatalf("bad test query %q: %v", query, err)
	}
	q, err := filter.Parse(values, filter.Options{Model: sqlb.ModelOf[Doc]()})
	if err != nil {
		t.Fatalf("Parse(%q): %v", query, err)
	}
	return filter.Apply(sqlb.Query[Doc]().OrderBy(sqlb.F("title").Asc()), q)
}

func TestJSONContainmentRunsAgainstPostgres(t *testing.T) {
	ctx := context.Background()
	raw := freshDB(t)
	applySchema(t, raw, docsRegistry())
	seedDocs(t, raw)

	docs, err := docsQuery(t, `metadata=contains.{"lang":"de"}`).All(ctx, sqlb.New(raw))
	if err != nil {
...
func TestJSONColumnScansAsADocument(t *testing.T) {
	...
	docs, err := docsQuery(t, `metadata=contains.{"tier":"pro"}`).All(ctx, sqlb.New(raw))
	if err != nil {
...
func TestJSONContainmentCanUseTheGINIndex(t *testing.T) {
	...
	sqlText, args, err := docsQuery(t, `metadata=contains.{"lang":"de"}`).SQL()
	if err != nil {
		t.Fatalf("SQL(): %v", err)
	}
