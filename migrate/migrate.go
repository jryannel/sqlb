...
type Options struct {
	...
	// Handwritten skips Header on this render. Use it for a Migration whose
	// Changes were composed by hand rather than produced by Diff — a trigger
	// function is the common case, and docs/migrations/README.md's worked
	// example (and example/tasks/cmd/migrate, example/fxapp/cmd/migrate) write
	// such Changes in Go and render them through this same package so they get
	// the same formatting, splitting and StatementBegin/End handling as
	// generated SQL.
	//
	// Rendering them with Header would be a false claim: `sqlb check`
	// (issue #178) trusts a header-bearing file enough to scan it for DDL
	// sqlb's own emitters never write, and flags one that has any — which a
	// hand-composed migration legitimately does. Handwritten is how such a
	// file says, honestly, that it was never sqlb's to make that claim about.
	Handwritten bool
}
...
// Header is the provenance comment every rendered migration carries, unless
// Options.Handwritten says not to stamp it. `sqlb check` (issue #178) reads it
// back: a file that has it is a claim that sqlb wrote everything in it, and
// the check scans such files for DDL sqlb's own emitters never write — a
// trigger, most often, since that is exactly what the schema DSL cannot
// express.
