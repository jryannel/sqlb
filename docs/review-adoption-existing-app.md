...
| 🔴 C | `rest` requires huma | **Fixed.** `rest` is its own module as of 2026-07-30, so importing the engine inherits neither huma nor its toolchain floor. Also narrower than feared: chi left the library graph in [#32](https://github.com/jryannel/sqlb/pull/32), and `rest.NewServer` builds the API on `net/http` — so the `humachi.New(...)`-over-existing-chi mount this report describes is no longer the shape. huma stays, deliberately: both escapes were scoped in full on 2026-07-30 and declined ([ADR-0007](adr/0007-generated-rest-handlers.md)) |
