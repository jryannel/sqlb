...
- **Confidence:** Medium
- **Decided:** 2026-07-27
- **Last reviewed:** 2026-07-30 (`rest` split into its own module; see Revisions)
...
keeps the engine's dependency claim intact: `rest` depends on huma, and the
engine depends on neither — which since the module split below is a statement
about `go.mod` files rather than about import edges.
...
write). The paths weighed are under *Alternatives considered*.

Keeping huma is what made the module split below the right answer instead: the
complaint was never "huma is bad", it was "I inherit it without asking", and a
boundary answers that without re-owning any of the machinery.

**`rest` is a Go module of its own** — `github.com/jryannel/sqlb/rest`, with the
blog example that mounts it in a third, `example/blog`. This reverses a rejection
this record made twice, and the reasoning is below under *Alternatives*. The
import path is unchanged: a consumer still writes
`import "github.com/jryannel/sqlb/rest"`, and what changes is that they also
write a `require` for it.

What the split buys is not a tidier tree. It is that the root `go.mod` now has
**no requirements at all** — three lines, no `go.sum` — so "importing sqlb costs
nothing" stops being a claim about the compile step that a module graph
contradicts, and becomes true where consumers actually feel it.
...
- ~~If huma in the module graph becomes a hard adoption blocker rather than a
  ...
  transitive dependency — the nested `rest` module reappears.~~ **Fired, and
  acted on.** It never became a *refusal*; what it became was measurable — huma's
  `go` directive setting the toolchain floor for consumers who never mount REST.
  The split is the Decision above. This trigger is spent.
- **If the two-tag tax bites the way both rejections predicted** — a consumer
  pins `sqlb` and `sqlb/rest` to versions that disagree, and the failure reads as
  a sqlb bug rather than as a skew — the answer is not to re-merge the modules.
  It is to make the skew loud: a build-tag assertion, or a documented
  compatibility line per release. Re-merging would restore the toolchain floor,
...
**The module split has its own cost of change, and it is asymmetric.** Splitting
was cheap: no `.go` file changed, because the import path `…/sqlb/rest` is
identical whether `rest` is a package in the root module or a module of its own.
What moved was five `go.mod` files, the `blogschema` package out from under the
new boundary, and the gates. Un-splitting is *not* the mirror of that, once
`rest` has a tag: consumers have `require github.com/jryannel/sqlb/rest` lines
that would have to come out, and a module path that stops existing is a broken
build rather than a deprecation. Before either module is tagged this is free; the
window closes at 1.0, which is why it happened now.
...
**A nested Go module for `rest`.** ~~Rejected twice.~~ **Taken, on 2026-07-30**,
and it is now the Decision above rather than an alternative. The history is kept
because the two rejections were not wrong when they were made, and what changed
was evidence rather than taste.

It was rejected first on the reasoning [ADR-0013](0013-no-internal-split.md)
gives — two modules to version and release in lockstep is a standing tax, and
`deps-check` already proved per package what the module boundary would prove by
construction. Rejected again the same day under adoption feedback, on the sharper
point that unlike `pgtest`, an internal test module nobody imports, a *published*
`rest` module pays that tax for real: two release tags in lockstep, and a version
skew a consumer can pin themselves into.

Both rejections rested on one belief — that the module boundary would only
restate what `deps-check` already enforced. **Measuring the `go` directive showed
that was false.** huma v2.39.0 declares `go 1.25.0`, so while `rest` shared this
module, 1.25 was the floor for every consumer of the engine, which needs only
1.24 and whose non-test code compiles at 1.21. A package-level check cannot see
that and cannot fix it: no amount of proving that `filter` imports nothing
third-party stops huma's toolchain requirement from reaching someone who never
mounts a REST surface. That is a constraint on the consumer's *build*, not on
their binary, and the only thing that removes it is the boundary.

The tax is real and is now paid rather than avoided: `rest` carries its own
version and its own tag, and a consumer can skew the two. What makes that
acceptable is that the alternative was a floor nobody could opt out of.
...
- 2026-07-27 — Written.
- 2026-07-30 — **`rest` split into a module of its own, reversing this record
  twice over.** The rejection stood on the belief that the boundary would only
  restate what `deps-check` already enforced per package. Measuring the `go`
  directive disproved it: huma's `go 1.25.0` was the toolchain floor for every
  consumer of an engine that needs 1.24, and no package-level check can see or
  fix that. `example/blog` became a third module for the same reason — it mounts
  `rest`, so leaving it in the root would have put huma back through the back
  door — and `example/blogschema` moved up a level, since a schema declaration is
  huma-free by nature and only its position inside `example/blog` coupled it.

  The result is a root `go.mod` of three lines with no requirements and no
  `go.sum`. `deps-check` is rewritten around that: the exemption list is empty,
  the module-graph assertion is "requires nothing" rather than "requires exactly
  huma", and both ADR-0016 positive controls moved to the `rest` module, since
  the root no longer has a dependency for them to find. Both were proven to fire.

  No `.go` file changed: `…/sqlb/rest` is the same import path either way. What
  the split costs is the two-tag skew both earlier rejections named, now paid
  rather than avoided.
- 2026-07-30 — Measured the module-graph cost the two ports complained about, and
  ...
  than the version-bump one. Recorded here as an accepted cost; acted on within
  the day, in the revision above.
