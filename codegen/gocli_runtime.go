...
// cliRuntime is the part of the emitted CLI that does not depend on the
// schema: the request type, the transport seam, the problem document, and the
// two things a caller would otherwise hand-write badly — the cursor walk and
// the rendering of an error that names what would have been accepted.
//
